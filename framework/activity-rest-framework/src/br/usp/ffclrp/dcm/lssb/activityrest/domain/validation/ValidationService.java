...
/**
 * A Service to validate an AnalysisActivity Instance based on its constraints.
 * 
 * @author cawal
 *
 */
public class ValidationService {
	...
	public static boolean isReady(AnalysisActivity aa) {
		...
		boolean parametersReady = aa.getDescription().getParameters().stream()
				.allMatch(d -> {
					...
					if (p instanceof List) {
						List l = (List) p;
						return MultiplicityElementUtil
								.validCardinality(l.size(), d);
					} else if (p != null) {
		...
		boolean inputsReady = aa.getInputs().stream().allMatch(d -> {
			...
			return MultiplicityElementUtil.validCardinality(cardinality,
					d.getDescription());
		});
	...
	public static ValidationResult validateParameters(
		...
		// for each parameter, get the constraints an apply
		// returns false
		ParameterMap parameters = activityInstance.getParameters();
		
		for (String key : parameters.keySet()) {
			Optional<Parameter> parameterDescriptionOp =
					parameterDescriptions.stream()
